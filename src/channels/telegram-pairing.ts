...
export interface WaitForPairingOptions {
  ...
  /**
   * Cancel the wait. When the signal aborts, the watcher + interval are
   * torn down and the promise rejects with reason `signal.reason`. Without
   * a signal the promise waits indefinitely (codes don't expire) — callers
   * who don't pass one must not reject the promise via other means or the
   * watcher and interval will leak.
   */
  signal?: AbortSignal;
}
