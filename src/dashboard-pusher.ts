...
// Surface the first error since the last success so a wedged dashboard isn't
// invisible — but stay rate-limited to avoid log floods if it stays down.
let lastPostErrorLogAt = 0;
const POST_ERROR_LOG_INTERVAL_MS = 60_000;
...
function postJson(config: PusherConfig, urlPath: string, data: unknown): void {
  ...
  req.on('error', (err) => {
    const now = Date.now();
    if (now - lastPostErrorLogAt > POST_ERROR_LOG_INTERVAL_MS) {
      lastPostErrorLogAt = now;
      log.warn('Dashboard POST failed', { urlPath, err });
    }
  });
  req.on('timeout', () => req.destroy(new Error('postJson timed out')));
  req.write(body);
  req.end();
}
...
function startLogTail(config: PusherConfig): void {
  ...
  // Backfill the last 200 lines, reading only the tail of the file. Reading
  // the full log on every restart was O(file size) — with no log rotation
  // it grew with uptime and slowed boot.
  try {
    ...
    fs.closeSync(fd);
    logOffset = stat.size;
    // If we sliced mid-line, drop the (probably-truncated) first line.
    const lines = buf.toString().split('\n').filter((l) => l.trim());
    const sliced = start > 0 ? lines.slice(1) : lines;
    const tail = sliced.slice(-200).map((l) => l.replace(ANSI_RE, ''));
...
// A malformed container_config used to throw straight out of JSON.parse,
// which blew up the whole snapshot. Now: log + treat as null so the rest
// of the dashboard payload is still useful.
function parseContainerConfig(agentGroupId: string, raw: string | null): unknown {
  if (!raw) return null;
  try {
    return JSON.parse(raw);
  } catch (err) {
    log.warn('container_config JSON parse failed; treating as null', { agentGroupId, err });
    return null;
  }
}
...
function collectAgentGroups() {
  return getAllAgentGroups().map((g) => {
    ...
    return {
      ...
      container_config: parseContainerConfig(g.id, (g as unknown as Record<string, string | null>).container_config),
...
// Per-file cache: stable across snapshots, reparsed only when mtime changes.
// scanJsonlTokens used to re-parse every JSONL line every minute; with many
// long sessions that pinned the event loop on the main thread.
type TokenEntry = { model: string; inputTokens: number; outputTokens: number; cacheReadTokens: number; cacheCreationTokens: number };
const tokenFileCache = new Map<string, { mtimeMs: number; entries: TokenEntry[] }>();
...
function scanJsonlTokens(agentDir: string) {
  ...
  const parseFile = (full: string): TokenEntry[] => {
    ...
    try {
      for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) walk(full);
          ...
          const cached = tokenFileCache.get(full);
          if (cached && cached.mtimeMs === mtimeMs) {
            entries.push(...cached.entries);
            continue;
          }
          const fresh = parseFile(full);
          tokenFileCache.set(full, { mtimeMs, entries: fresh });
          entries.push(...fresh);
        }
      }
    } catch { /* skip dir */ }
